...
correlativas_plan(X) :-
	materias(X,_,_,_),
	correlativa(X,Y),
	((length(Y,0), write(', esta materia no tiene correlativas.'));
	(not(length(Y,0)), write(' y las correlativas son: '), mostrar_correlativas(Y))).
...
plan_is :-
	...
	materias(Cod,Nombre,Anio,Cua),
	nl,
	write('Anio:'),write(Anio),
	write(' Cua:'),write(Cua),
	write(' Nom:'),write(Nombre),
	correlativas_plan(Cod),
	nl,fail;nl.

plan_is_lista(M):- findall(X,(materias(_,X,_,_)),M).

materias_de(A,M):- findall(Nom,(materias(_,Nom,A,_)),M).
